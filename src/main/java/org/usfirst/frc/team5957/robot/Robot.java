...
public class Robot extends IterativeRobot {
	...
	private Gripper gripper = new Gripper(4, 5, PCM, 0);
	private Lift lift = new Lift(6);
	...
	final double AUTO_MAX_SPEED = 0.7;
	final double TELE_LOW_MAX = 1; // TODO : test this
	final double TELE_HIGH_MAX = 0.9; // TODO : test this
	final double TELE_LOW_ROTATION = 1; // TODO : test this
	final double TELE_HIGH_ROTATION = 0.9; // TODO : test this
	...
	public void autonomousInit() {
		drive.setMaxOutput(AUTO_MAX_SPEED);
		...
		if (priority.get() == Switch) { // Priority is switch
			...
			if (scalePos != startPosition && switchPos == startPosition) { // If we can't do scale
				switchAuto();
	...
	// Actual Autos
	public void scaleAuto() { // DO NOT USE
	...
	public void scaleAutoOS() { // DO NOT USE
	...
	public void centerSwitchAuto() { // THIS WORKS
		if (action == 1) { // Initial forward
	...
	public void switchAuto() { // FIX ME, please test
		if (action == 1) {
	...
	public void straightAuto() { // THIS WORKS
		if (action == 2) {
	...
	public void teleopPeriodic() {
		...
		// Drive gears
		if (pressed(driver, ControlMap.switchGear)) {
			setHighGear();
		} else {
			setLowGear();
		}
		...
		// Drive control
		double speed = maxSpeed * -driver.getRawAxis(ControlMap.speedAxis);
		double rotation = maxRotation * driver.getRawAxis(ControlMap.rotationAxis);
		// boolean quickTurn = pressed(driver, ControlMap.quickTurn);

		drive.arcadeDrive(speed, rotation, true);

		// Lift
		if (operator.getRawAxis(ControlMap.lift) > DEADZONE) {
			lift.climb(operator.getRawAxis(ControlMap.lift));
		} else if (operator.getRawAxis(ControlMap.lift) < -DEADZONE) {
			lift.descend(operator.getRawAxis(ControlMap.lift));
		} else {
			lift.stall();
		}
		...
		// Gripper
		if (pressed(operator, ControlMap.openGripper)) {
			gripper.open();
		} else {
			gripper.close();
		}
	...
	public void setHighGear() {
		gear.set(false);
		maxSpeed = TELE_HIGH_MAX;
		maxRotation = TELE_HIGH_ROTATION;
	}

	public void setLowGear() {
		gear.set(true);
		maxSpeed = TELE_LOW_MAX;
		maxRotation = TELE_LOW_ROTATION;
	}

	// private double getAdjusted(double speed) {
	// return speed * Math.pow(Math.abs(speed), 3);
	// }
}